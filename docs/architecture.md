...
code. The runtime owns generic HTTP and lifecycle policy; the generated
...
- `runtime` is the default. Generic lifecycle, HTTP middleware, responses,
  validation, and security primitives come from the pinned gin-kit module.
- `standalone` contains the infrastructure implementation in the generated
  project for developers who want a standalone, source-visible reference.

Both project types keep routes, handlers, services, domains, repositories,
migrations, configuration, and selected database integration explicit.

## Runtime foundation

The runtime application validates configuration, opens the selected SQL
...
API handlers use the runtime response and validation packages. Public error
