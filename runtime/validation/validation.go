...
func englishMessage(c Context) string {
	...
	case "lte":
		return fmt.Sprintf("The %s field must not be greater than %s.", c.Field, c.Parameter)
	case "gt":
		return fmt.Sprintf("The %s field must be greater than %s.", c.Field, c.Parameter)
	case "lt":
		return fmt.Sprintf("The %s field must be less than %s.", c.Field, c.Parameter)
	case "url":
		return fmt.Sprintf("The %s field must be a valid URL.", c.Field)
	case "uuid":
		return fmt.Sprintf("The %s field must be a valid UUID.", c.Field)
	case "numeric":
		return fmt.Sprintf("The %s field must be a number.", c.Field)
	case "alphanum":
		return fmt.Sprintf("The %s field must only contain letters and numbers.", c.Field)
	case "datetime":
		return fmt.Sprintf("The %s field must match the %s format.", c.Field, c.Parameter)
	case "eqfield":
		return fmt.Sprintf("The %s field must match the %s field.", c.Field, c.Parameter)
	case "ne":
		return fmt.Sprintf("The %s field must not be %s.", c.Field, c.Parameter)
	case "startswith":
		return fmt.Sprintf("The %s field must start with %s.", c.Field, c.Parameter)
	case "endswith":
		return fmt.Sprintf("The %s field must end with %s.", c.Field, c.Parameter)
	default:
