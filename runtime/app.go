// Package runtime provides gin-kit's explicit application lifecycle and
...
import (
	...
	"github.com/Alfian57/gin-kit/runtime/cache"
	runtimedb "github.com/Alfian57/gin-kit/runtime/database"
	"github.com/Alfian57/gin-kit/runtime/devtools"
	"github.com/Alfian57/gin-kit/runtime/httpx"
	"github.com/Alfian57/gin-kit/runtime/metrics"
...
func New(options Options) (*Application, error) {
	...
	if options.HTTP.RateLimit.Enabled && options.HTTP.RateLimit.RequestsPerMinute < 1 {
		return nil, errors.New("runtime: rate limit requests per minute must be positive")
	}
	if options.Docs.Enabled {
		if (options.Docs.BasicAuthUsername == "") != (options.Docs.BasicAuthPassword == "") {
			return nil, errors.New("runtime: docs basic auth requires both a username and a password")
		}
		if options.Docs.Path == options.Docs.SpecPath {
			return nil, errors.New("runtime: docs path and spec path must differ")
		}
		if !strings.HasPrefix(options.Docs.Path, "/") || !strings.HasPrefix(options.Docs.SpecPath, "/") {
			return nil, errors.New("runtime: docs paths must start with /")
		}
	}
	if options.DevTools.Enabled {
		...
		if options.Environment != "development" {
			return nil, errors.New("runtime: devtools must not be enabled outside development")
		}
		if !strings.HasPrefix(options.DevTools.Path, "/") {
			return nil, errors.New("runtime: devtools path must start with /")
		}
	}
	...
	if err := router.SetTrustedProxies(options.HTTP.TrustedProxies); err != nil {
		return nil, fmt.Errorf("runtime: invalid trusted proxies: %w", err)
	}
	...
	var connection *runtimedb.Connection
	if options.Database != nil {
		var err error
		connection, err = runtimedb.Open(context.Background(), *options.Database)
		if err != nil {
			return nil, err
		}
	}
...
func resolveRedisOptions(options *Options) (*redis.Options, error) {
	...
	case "redis":
	default:
		return nil, fmt.Errorf("runtime: unknown cache driver %q", options.Cache.Driver)
	}
	if options.Cache.RedisURL == "" {
		return nil, errors.New("runtime: the redis cache driver requires a Redis URL")
	}
	parsed, err := redis.ParseURL(options.Cache.RedisURL)
	if err != nil {
		return nil, fmt.Errorf("runtime: invalid Redis URL: %w", err)
	}
	return parsed, nil
}
...
func (a *Application) installQueue(options Options, jobs *queue.Queue) error {
	...
	if _, exists := a.readiness["redis"]; !exists {
		...
		if err != nil {
			return fmt.Errorf("runtime: invalid Redis URL: %w", err)
		}
...
func (a *Application) Database() *runtimedb.Connection { return a.database }
	...
	for _, item := range runners {
		group.Go(func() error {
			...
			if err != nil && !errors.Is(err, context.Canceled) {
				return fmt.Errorf("runtime: runner %q: %w", item.name, err)
			}
			return nil
		})
	}
...
func waitForGroup(ctx context.Context, group *errgroup.Group) error {
	...
	case err := <-done:
		return err
	case <-ctx.Done():
		return errors.New("runtime: runners did not exit before the shutdown timeout")
	}
}
